...
export class CatSinglePage implements OnInit {
    ...
    // 房型頁面的排序選項
    roomSortOptions: SortOption[];
    selectedRoomSortOption: SortOption | undefined;
    ...
    getSortedRooms(): Room[] {
        ...
        const rooms = [...this.hotelDetail.rooms]; // 複製陣列，避免修改原始資料

        if (!this.selectedRoomSortOption) {
            return rooms; // 如果沒有選擇排序，返回原始順序
        }

        switch (this.selectedRoomSortOption.code) {
            case 'price_asc':
                // 價格由低到高
    ...
    /**
     * 生成房型數量下拉選單的選項
     * @param totalUnits 剩餘房間數量
     * @returns 數量選項陣列（0 到剩餘數量）
     */
    getRoomQuantityOptions(totalUnits: number): number[] {
        const max = Math.min(totalUnits, 10); // 最多顯示 10 間或剩餘數量
        return Array.from({ length: max + 1 }, (_, i) => i); // 0, 1, 2, ..., max
    }
    ...
    private prepareOrderData(): OrderData | null {
        ...
        this.hotelDetail.rooms.forEach((room, index) => {
            const quantity = this.roomSelections[index];
            if (quantity && quantity > 0) {
                selectedRooms.push({
                    ...
                    roomTotal: room.basePrice * quantity,
                    expanded: false
                });
            }
        });
