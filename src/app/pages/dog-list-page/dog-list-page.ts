...
@Component({
    ...
    styleUrl: './dog-list-page.css'
})
export class DogListPage implements OnInit, OnDestroy {
    ...
    onSearch() {
        ...
        if (this.searchParams.petType !== 'DOG') {
            console.log(`⚠️ petType 已改變為 ${this.searchParams.petType}，導向對應頁面`);
            const targetRoute = this.searchParams.petType === 'CAT' ? '/catHotels' : '/dogHotels';
    ...
    performSearch() {
        ...
        console.log('=== Dog List Page - 執行搜尋 ===');
    ...
    navigateToDetail(propertyId: string): void {
        ...
        this.hotelService.querySingleHotelDetail(apiParams).subscribe({
            next: (response) => {
                ...
                if (response.MWHEADER.RETURNCODE === '0000') {
                    // 成功，導航到狗狗旅館詳情頁，使用 query parameter 傳遞 propertyId
                    this.router.navigate(['/dogSingleHotel'], {
